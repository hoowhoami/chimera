...
mod tests {
    ...
    impl TestPropertySource {
        ...
        fn with_property(mut self, key: &str, value: ConfigValue) -> Self {
    ...
    fn test_environment_basic() {
        ...
        let source = TestPropertySource::new("test", 0)
            .with_property("app.name", ConfigValue::String("TestApp".to_string()))
            .with_property("app.port", ConfigValue::Int(8080));

        env.add_property_source(Box::new(source));

        assert_eq!(env.get_string("app.name"), Some("TestApp".to_string()));
    ...
    fn test_environment_priority() {
        ...
        let low = TestPropertySource::new("low", 1)
            .with_property("key", ConfigValue::String("low".to_string()));

        let high = TestPropertySource::new("high", 10)
            .with_property("key", ConfigValue::String("high".to_string()));

        env.add_property_source(Box::new(low));
        env.add_property_source(Box::new(high));

        // 高优先级的值应该被返回
        assert_eq!(env.get_string("key"), Some("high".to_string()));
    }

    #[test]
    fn test_config_value_conversions() {
        let str_val = ConfigValue::String("123".to_string());
        assert_eq!(str_val.as_i64(), Some(123));
        assert_eq!(str_val.as_f64(), Some(123.0));
    ...
    #[test]
    fn test_env_property_source() {
        std::env::set_var("APP_DATABASE_URL", "postgres://localhost");
        std::env::set_var("APP_SERVER_PORT", "8080");

        let source = EnvironmentPropertySource::new("APP_");

        assert_eq!(
            source.get("database.url").and_then(|v| v.as_str().map(String::from)),
            Some("postgres://localhost".to_string())
        );
        assert_eq!(
            source.get("server.port").and_then(|v| v.as_str().map(String::from)),
            Some("8080".to_string())
        );
    }

    #[test]
    fn test_toml_property_source() {
        let toml_content = r#"
            [database]
        ...
        let source = TomlPropertySource::from_str(toml_content, "test.toml".to_string()).unwrap();

        assert_eq!(
            source.get("database.url").and_then(|v| v.as_str().map(String::from)),
            Some("postgres://localhost".to_string())
        );
        assert_eq!(source.get("database.port").and_then(|v| v.as_i64()), Some(5432));
        assert_eq!(source.get("database.enabled").and_then(|v| v.as_bool()), Some(true));
    }

    #[test]
    fn test_map_property_source() {
        let source = MapPropertySource::new("test")
            .with_property("key1", ConfigValue::String("value1".to_string()))
            .with_property("key2", ConfigValue::Int(42));

        assert_eq!(
            source.get("key1").and_then(|v| v.as_str().map(String::from)),
            Some("value1".to_string())
        );
        assert_eq!(source.get("key2").and_then(|v| v.as_i64()), Some(42));
    }
}
